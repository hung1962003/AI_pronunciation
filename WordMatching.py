...
def getWhichPhomenesWereTranscribedCorrectly(real_ipa, recorded_ipa):
    is_leter_correct = [None]*len(real_ipa)    
    for idx, letter in enumerate(real_ipa):   
        letter = letter.lower()    
        recorded_ipa[idx] = recorded_ipa[idx].lower() 
        if letter == recorded_ipa[idx] or letter in punctuation:
            is_leter_correct[idx] = 1
        else:
            is_leter_correct[idx] = 0
    return is_leter_correct
