...
let STScoreAPIKey = ""; //"B6JZgqCcRE44mhJsutqD8ZHbRitI2M21AZS1P3pf"; //'rll5QsTiv83nti99BW6uCmvs9BDVxSB39SVFceYb'; // Public Key. If, for some reason, you would like a private one, send-me a message and we can discuss some possibilities
let apiMainPathSample ="";// "https://ai.aespwithai.com"; // 'http://127.0.0.1:3001';// 'https://a3hj0l2j2m.execute-api.eu-central-1.amazonaws.com/Prod';
let apiMainPathSTS ="";//"https://ai.aespwithai.com"; //"https://jv098fghxg.execute-api.ap-southeast-1.amazonaws.com/default"; //"https://61rcdxiwhf.execute-api.ap-southeast-1.amazonaws.com/Prod"; // 'https://wrg7ayuv7i.execute-api.eu-central-1.amazonaws.com/Prod';
...
const cacheSoundFiles1 = async () => {
  const files = ["ASR_good.wav", "ASR_okay.wav", "ASR_bad.wav"];
  try {
    const buffers = await Promise.all(
      files.map((name) => loadOneSound(`${soundsPath}/${name}`))
    );
    [soundFileGood, soundFileOkay, soundFileBad] = buffers;
    console.log("Tất cả sound files đã được preload và decode.");
  } catch (err) {
...
const changeLanguage = (language, generateNewSample = false) => {
  ...
  switch (language) {
    case "en-gb":
      document.getElementById("languageBox").innerHTML = "English-UK";
      languageIdentifier = "en-GB";
      languageName = "Google UK English Female";
      break;

    case "en":
      document.getElementById("languageBox").innerHTML = "English-USA";
      languageIdentifier = "en-US";
      languageName = "Microsoft David - English (United States)";
      break;
  }
  ...
  for (idx = 0; idx < voices.length; idx++) {
    if (
      voices[idx].lang
        .toLowerCase()
        .startsWith(languageIdentifier.toLowerCase()) &&
        ...
        try {
          ...
          await fetch(apiMainPathSTS + "/GetAccuracyFromRecordedAudio", {
            ...
            .then((data) => {
              ...
              console.log(lettersOfWordAreCorrect);

              startTime = data.start_time;
              endTime = data.end_time;

              real_transcripts_ipa = data.real_transcripts_ipa.split(" ");
              console.log("real_transcripts_ipa : " + real_transcripts_ipa);
  ...
  while (!valid_response) {
    ...
    try {
      await fetch(apiMainPathSTS + "/GetAccuracyFromRecordedAudio", {
        ...
        headers: { "X-Api-Key": STScoreAPIKey },
      }).then((valid_response = true).then(console.log));
